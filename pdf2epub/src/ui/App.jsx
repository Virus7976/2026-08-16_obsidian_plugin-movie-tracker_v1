...
export default function App() {
  ...
  // A PDF shared into the installed app is parked in a cache by the service worker.
  useEffect(() => {
  ...
  const clearDone = () => {
    setJobs((prev) => {
      prev.filter((j) => j.status === 'done' && j.url).forEach((j) => URL.revokeObjectURL(j.url));
      ...
      <header className="masthead">
        ...
        {installEvent && (
          <button
            type="button"
            className="ghost"
            onClick={async () => { installEvent.prompt(); setInstallEvent(null); }}
          >
            Install app
          </button>
        )}
          ...
          <section className="jobs">
            <div className="jobs-head">
              ...
              {jobs.some((j) => j.status === 'done' || j.status === 'error') && (
                <button type="button" className="ghost small" onClick={clearDone}>Clear finished</button>
              )}
            </div>
            {jobs.map((job) => (
              <JobCard key={job.id} job={job} onRead={() => setReading(job)} />
            ))}
          </section>
        )}
